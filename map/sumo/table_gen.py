...
dest_edges=[]
data={}
with open('/home/ubuntu/NewYork/map/tables/out.json') as data_file:    
    data = json.load(data_file)

weight_file=open("/home/ubuntu/NewYork/map/sumo/weights","w")
...
for dest in dest_edges:
    for each in data:
        ...
        f.close()
        try:
            f=open("sumo_tables/"+str(each),"a")
        except:
            f=open("sumo_tables/"+str(each),"w")
        try:
            os.system('duarouter --trip-files temp_trip --net-file map.net.xml --weight-files weights --output-file result_here.rou.xml ')
